...
Wayclip stores its configuration in a `settings.json` file. The default path is:

```bash
~/.config/wayclip/settings.json
```

<Callout title="Note">
You can access the config using your default editor via our CLI
...
- **Save Shortcut** – Quickly save a clip. Default: `Alt+C`.
...
These settings are mostly for developers or power users:
